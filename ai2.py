...
class MCTSNode:
    ...
    def simulate(self):
        ...
        while len(legal_moves) > 0:
            ...
            win,path = check_win(current_state,move,current_player,[])
            if(win):
        ...
        return 1 if np.sum(current_state) % 2 == 1 else 2,0

    def backpropagate(self, result,effecive):
        """
        ...
        if result == self.player_number:
            self.wins += effecive
        if self.parent:
            self.parent.backpropagate(result,effecive)
...
class AIPlayer:
    ...
    def uct_search(self, root: MCTSNode, time_limit=5):
        ...
        while time.time() < end_time:
            ...
            result,len_path = node.simulate()
            l,b = (node.state).shape
            
            node.backpropagate(result,l*b - len_path)
